...
# set -e  # Exit on any error - Temporarily commented out for debugging
...
for file in "${demo_files[@]}"; do
    if [ -f "$file" ]; then
        ...
        echo "ℹ️  Demo file $file not found, skipping." # Add a message for not found files
    fi
done

if [ $moved_count -eq 0 ]; then
    # Check if demos directory is empty and was created by this script run
    if [ -d "demos" ] && [ -z "$(ls -A demos)" ]; then
        echo "ℹ️  No demo files were moved and demos/ directory is empty, removing it."
        rmdir demos
    elif [ ! -d "demos" ]; then # If demos dir was removed because it was empty
