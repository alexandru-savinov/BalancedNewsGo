...
class ProgressIndicatorTests {
  ...
  async testProgressUpdates() {
    ...
    if (window.EventSource.lastInstance) {
      window.EventSource.lastInstance.simulateProgress(42, 'processing', 'Analyzing sentiment');
      await this.wait(50);
    }
  ...
  async testReconnectionLogic() {
    ...
    if (window.EventSource.lastInstance) {
      window.EventSource.lastInstance.simulateError();
      await this.wait(100);
    }
...
// Store the last created EventSource instance for testing
let originalEventSource = window.EventSource;
window.EventSource = class extends MockEventSource {
  constructor(url) {
    super(url);
    window.EventSource.lastInstance = this;
  }
};
