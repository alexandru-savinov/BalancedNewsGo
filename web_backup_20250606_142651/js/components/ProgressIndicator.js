...
class ProgressIndicator extends HTMLElement {
  ...
  async connect(articleId) {
    ...
    try {
      ...
      this.#sseClient.addEventListener('completed', (data) => {
        ...
        this.#dispatchEvent('connectionerror', { message: data.message || 'Connection error occurred', data });
      });
  ...
  updateProgress(progressData) {
    ...
    // Handle status change properly to emit statuschange events
    if (progressData.status && progressData.status !== oldStatus) {
      this.#updateStatus(progressData.status, progressData.stage || this.#stage);
    } else if (progressData.stage && progressData.stage !== this.#stage) {
      this.#stage = progressData.stage;
    }
